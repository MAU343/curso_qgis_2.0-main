...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showPreviousRating(dia) {
    ...
    previousRating.innerHTML = '<div class="font-mono small text-qgis-yellow mb-1">Tu valoración:</div>' + starsHtml;
  }
  ...
  function actualizarEstadoUI() {
    if (yaComento(DIA_GENERAL)) {
      formWrapper.classList.add('d-none');
      alreadyWrapper.classList.remove('d-none');
      showPreviousRating(DIA_GENERAL);
    } else {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch(COMMENTS_API, {
        ...
        body: JSON.stringify({ dia: DIA_GENERAL, content, rating })
      });

      if (!res.ok) throw new Error();

      marcarComentado(DIA_GENERAL, rating);
      form.reset();
      resetRating();
      actualizarEstadoUI();

      submitStatus.className = 'alert alert-success rounded-0 border-0 font-mono small mt-3 animate-fade-in';
      submitStatus.innerHTML = '<i class="bi bi-check-circle-fill me-2"></i> Comentario y valoración enviados correctamente.';
